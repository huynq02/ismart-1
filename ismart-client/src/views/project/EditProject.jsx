...
const ModelEditProject = ({ isShow, handleClose, dataUpdateSupplier, updateTableSupplier }) => {

    const [nameSupplier, setNameSupplier] = useState("");


    useEffect(() => {
        setNameSupplier(dataUpdateSupplier.projectName ? dataUpdateSupplier.projectName : "");
    }, [dataUpdateSupplier])
    ...
    const handleSave = async () => {
        if (!validateTextRequired.test(nameSupplier)) {
            ...
            let res = await editProject(dataUpdateSupplier.projectId, removeWhiteSpace(nameSupplier));
            console.log(res);
            if (res) {
    ...
    const handleReset = () => {
        setNameSupplier(dataUpdateSupplier.projectName);
    }

    return (<>
        <Modal show={isShow} onHide={handleCloseModal}>
            <Modal.Header closeButton>
                <Modal.Title>Sửa tên dự án</Modal.Title>
            </Modal.Header>
            <Modal.Body>
...
export default ModelEditProject;